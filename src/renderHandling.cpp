        ...
        for (int x = 0; x<pixelDimensions.x; ++x)
        {
            for (int y = 0; y<pixelDimensions.y; ++y)
            {
                ...
                if (bary)
                {
                    ...
                    float temp = PerspBarycentricInterp(*bary, v1.z, v2.z, v3.z);
                    std::cout<<"Pixel: "<<x<<" "<<y<<" |  PerspBarycentricInterp: "<<temp<<"\n";
...
{
    std::cout<<"Testing PixelToScreenSpace\n";
    float2 newCoords;
    float2 pixelDimensions = camera.GetPixelDimensions();
    ...
    std::cout<<"NewCoords: "<<newCoords.x<<" "<<newCoords.y<<"\n";
    return newCoords;
}
